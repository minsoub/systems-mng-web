...
export default function GroupSelect({ items, title, value, onChange }) {
    return (
            ...
            <Select id="grouped-select" labelId="grouped-select" name="grouped-select" value={value} onChange={onChange}>
                {items.map((item) => {
                    ...
                    if (children.length) {
                        return (
                            <div key={id}>
                                <MenuItem sx={{ color: '#8C8C8C' }} value={id}>
                                    {name}
                                </MenuItem>
                                ,
                                {children.map((subItem) => (
                                    <MenuItem sx={{ paddingLeft: 5 }} key={subItem.id} value={subItem.id}>
                                        {subItem.name}
                                    </MenuItem>
                                ))}
                            </div>
