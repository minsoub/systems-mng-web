...
const ServiceLogsApi = () => {
    ...
    const getLrcLogExcelExport = (fromDate, toDate, keyword, reason) => {
        const encodeKeyword = encodeURIComponent(keyword);
        const encodeReason = encodeURIComponent(reason);
        callApi('logExport', {
            axiosInstance: axiosInstanceDownload,
            method: 'get',
            url: `/mng/lrc/service/logs/excel/export?fromDate=${fromDate}&toDate=${toDate}&keyword=${encodeKeyword}&reason=${encodeReason}`,
            requestConfig: {}
        });
    };
