...
import { setSearchData } from 'store/reducers/cpc/EventSearch';
...
const View = () => {
    ...
    const { reduceFromDate, reduceToDate, reducePeriod, reduceKeyword } = useSelector((state) => state.cpcEventSearchReducer);
    ...
    const handleClick = (rowData) => {
        if (rowData && rowData.field && rowData.field !== '__check__') {
            navigate(`/cpc/contents/event/reg/${rowData.id}`);
        }
    };
    ...
    const addClick = () => {
        ...
        navigate('/cpc/contents/event/reg');
    };
